...
data$CO2_mol_per_m3 <- ppm_to_mol(data$CO2_tracer_gam,"ppm",p_kPa = data$PressureActual_hPa/10,T_C = data$T_soil)
...
injection_rates <- sapply(data_sub,'[[',"inj_mol_m2_s")[1,]

min_DS <- c(2e-6,5e-7,1e-7)
max_DS <- c(9e-6,5e-6,3e-6)
...
CO2_mod_sweep <- readLines(paste0(comsolpfad,"sweep_Hartheim_3DS_mod_dates.txt"))
...
##########################################
#dottyplot
ggplot(subset(DS_long,rmse < sort(unique(rmse))[500]))+geom_point(aes(DS,rmse))+facet_wrap(~Schicht,scales="free")
...
sweep_plot <- ggplot(CO2_obs)+
  ...
  geom_point(aes(CO2_tracer_gam,tiefe,col="obs"))
