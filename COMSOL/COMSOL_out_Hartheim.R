...
colnames(DS_mat) <- pars
DS_wide <- cbind(rmse,DS_mat)
DS_long <- reshape2::melt(DS_wide, id = "rmse",variable="Schicht",value.name="DS")
ggplot(subset(DS_long,rmse < sort(unique(rmse))[100]))+geom_point(aes(DS,rmse))+facet_wrap(~Schicht,scales="free")
...
#DS_profil$DS[4] <- NA
data_agg$CO2_mod <- ppm_to_mol(CO2_sweep[,best.fit.id],"mol/m^3")

# ende for schleife


data_agg$DS <- approx(DS_profil$bottom,DS_profil$DS,data_agg$tiefe,method="constant",rule=2)$y
data_agg$DS_D0 <- approx(DS_profil$bottom,DS_profil$DS_D0,data_agg$tiefe,method="constant",rule=2)$y
data_agg$DS[data_agg$tiefe > 0] <- NA


# sweep_plot <- ggplot(sweep_long)+
#   geom_path(aes(CO2_mod,tiefe,col=par))+
#   geom_point(data=data_agg,aes(tracer,tiefe),col="black")+
#   geom_point(data=data_agg,aes(CO2_mod,tiefe),col="red")+
#   geom_hline(yintercept = schicht_grenzen)+
#   guides(col=F)
sweep_plot <- ggplot(data_agg)+
  geom_point(aes(tracer,tiefe,col="obs"))+
  geom_point(aes(CO2_mod,tiefe,col="mod"))+
  geom_hline(yintercept = schicht_grenzen)

DS_D0_plot <- ggplot(data_agg)+geom_path(aes(DS_D0,tiefe))
DS_plot <- ggplot(data_agg)+geom_path(aes(DS,tiefe))
...
slope_0_5cm <- glm(CO2_ref_1.5 ~ tiefe, data= subset(data_agg,tiefe <0 & tiefe > -5))#ppm/cm
