...
data_DS$Versuch[data_DS$Versuch == 8 & data_DS$step > 4] <- 9
data_DS$mode[data_DS$Versuch == 1 & data_DS$step <= 8] <- "2D PP"
data_DS$Versuch[data_DS$Versuch == 1 & data_DS$step > 8] <- 1.2
...
fm_1D <- glm(DSD0 ~ exp(PPC_meanr6) ,data=data_1D)
fm_2D <- glm(DSD0 ~ exp(PPC_meanr6) ,data=data_2D)
...
ggplot(subset(data_DS3,Versuch %in% Versuche_auswahl))+
  ...
  geom_point(aes(PPC_meanr6,DSD0,col=factor(Versuch,levels=Versuche_auswahl,labels=1:length(Versuche_auswahl))))+
  facet_wrap(~mode)+
