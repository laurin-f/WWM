...
DS_long[,paste0(DSD0_cols,"_aniso")] <- DS_long[,DSD0_cols] * 1.26
...
ggplot(subset(ds_sub,method=="drift"))+
  ...
  geom_line(aes(date,DSD0_max_roll,group=id,col="max"))+
  geom_line(aes(date,DSD0_min_roll,group=id,col="min"))+
  geom_line(aes(date,DSD0_roll,group=id))
###############################################################################
###############################################################################
...
DS_boxplot <- 
  ...
  geom_boxplot(data=subset(ds_soil,method=="SWC_T"),aes(fill=as.factor(method),col=method,ifelse(DSD0_roll_aniso > 1, 1,DSD0_roll_aniso),x=id),width=0.2,position = position_nudge(x = 0.25),alpha=0.3)+
  facet_wrap(~windy)+
...
flux_plot <- 
  ...
  scale_color_manual(values = rep(meth_col,2))+
...
col_labs <- c("DSD0","DeffD0" ,"DPPED0","PPC")
col_exps <- c(expression(D[S]/D[0]),expression(D[eff]/D[0]),expression(D[PPE]/D[0]),"PPC")


PPC_DS_plot <- 
  ggplot(subset(data,!is.na(Versuch)))+
  geom_ribbon(data=subset(ds_sub,method=="drift" & id == 1),aes(x=date,ymin=DSD0_min_roll_aniso,ymax=DSD0_max_roll_aniso,fill="DeffD0"),alpha=0.2,linetype=2)+
  geom_line(data=subset(PPC_DS,date %in% ds_sub$date),aes(x=date,y=base,col="DSD0"),linetype=2)+
  geom_line(data=subset(PPC_DS,date %in% ds_sub$date),aes(date,PPC,col="PPC"))+
  
  geom_line(data=subset(PPC_DS,date %in% ds_sub$date),aes(date,peak,col="DPPED0"))+
  geom_line(data=subset(PPC_DS,date %in% ds_sub$date),aes(date,DSD0_1,col="DeffD0"))+
...
ggplot(subset(ds_sub,method=="drift" & id ==1))+
  geom_line(aes(date,DSD0_roll),col="red")+
...
fm_PPC_DPPE <- glm(peak~PPC,data=subset(PPC_DS_sub,date %in% ds_sub$date))
...
DPPE_PPC <- 
  ...
  geom_point(aes(PPC,peak,col=method_labels[1],fill=method_labels[1]),alpha=0.8)+
  geom_smooth(aes(PPC,peak),method = "glm",se=F,col=1,lwd=0.6,linetype="dashed")+
