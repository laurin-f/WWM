...
datelim2 <- ymd_hms("2020.06.26 12:30:00","2020.06.30 15:30:00")
...
change2.2 <- ymd_hm("2020.06.30 09:00")
data$pos <- NA
data$pos[data$date > change1 + 1800 & data$date < change1.2] <- "2"
data$pos[data$date < change1 - 1800 | data$date > change1.2] <- "1"
...
ggplot(subset(data2,tiefe!=0))+
  geom_vline(xintercept = c(change2,change2.2))+
  annotate("text",x=c(change2,change2.2),y=Inf,label="vertauscht",hjust=0,vjust=1)+
  geom_line(aes(date,CO2_smp1,col="smp1"))+
  geom_line(aes(date,CO2_smp2,col="smp2"))+
