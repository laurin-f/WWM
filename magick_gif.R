...
data <- data %>% 
  group_by(id) %>%
  mutate(P_roll = RcppRoll::roll_mean(P,3/dt,fill=NA))

#messung i=9 am Anfang alle P Sonden zusammen


data_sub <- sub_daterange(data,ymd_hms("2022-04-12 13:00:00","2022-04-12 13:02:00"))
...
anim_save(paste0(plotpfad_PPchamber,"2D_PP_reveal.gif"),width=5,height=8)
#########################
#Magick
...
g <- ggplot(subset(data_sub_i,id %in% 1:4 ))+
  ...
  labs(x="",y="Pressure (Pa)",col="subchamber")+ggsave(file.path(dir_out, paste0(i, ".png")),width = 5,height=3)
