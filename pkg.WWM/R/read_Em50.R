...
update_Em50_data.db<-function(table.name="Em50"){
  ...
  if(file.exists(paste0(datapfad_schaui,"db_log.txt"))){
    files.old<-read.csv(paste0(datapfad_schaui,"db_log.txt"),stringsAsFactors = F)
  ...
  files<-list.files(paste0(datapfad_schaui,"Em50Data"),".*",full.names = T)
  ...
  if(length(files.new)>0){
    ...
    data<-lapply(files.new,function(x){
      ...
      if(dateityp==".xls"){
        ...
        line1<-readxl::read_xls(x,n_max=1)
        ...
        data.x<-readxl::read_xls(x,skip=2,col_types = c("date", rep("numeric",n_cols-1)))
    ...
    data_rbind<-lapply(unique(logger_ids),function(x){
      ...
      if(length(listen_id)>1){
        #diese Listenelemente auswählen
        logger.x <- data[listen_id]
    ...
    write.csv(files,
              paste0(datapfad_schaui,"db_log.txt"),row.names = F)
  }#ende if files.new > 0
}#ende Funktion
...
read.Em50<-function(format="long", ...){
                    #query="SELECT datetime(date,'unixepoch') AS datetime,* FROM Em50"){
  #
  # con<-dbConnect(RSQLite::SQLite(),paste0(sqlpfad,"Em50_data.db"))
