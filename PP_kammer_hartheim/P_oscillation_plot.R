...
data_PPC <- data_PPC %>% 
  ...
  mutate(dt = diff_time(date,"secs"),
         ...
         P_roll = RcppRoll::roll_mean(P,20*60,fill=NA))
...
data_PPC$P <- data_PPC$P - data_PPC$P_ref


ggplot(subset(data_PPC,date %in% round_date(date,"10 mins")))+
...
cal_period <- data_PPC %>% 
  ...
  mutate(zeit = as.numeric(difftime(date,min(date))),
         sensor_drift = predict(glm(P ~ zeit)),
...
data_PPC$mode <- NA
data_PPC$mode[daterange_id(data_PPC,c(step_df_sub$Start[1],step_df_sub$End[1]))] <- "PP & P-lateral"
data_PPC$mode[daterange_id(data_PPC,c(step_df_sub$Start[8],step_df_sub$End[8]))] <- "PP"
data_PPC_alt <- data_PPC %>% 
  filter(id %in% c(1,3) & !is.na(mode)) %>%
  group_by(mode,id) %>% 
  mutate(zeit = as.numeric(date - min(date))-3600,
         P_roll1 = RcppRoll::roll_mean(P_cal,10,fill=NA),
         P_roll3 = RcppRoll::roll_mean(P_cal,1200,fill=NA),
         P_roll2 = RcppRoll::roll_mean(P_cal,3600,fill=NA)) %>% 
  filter(zeit < 180 & zeit > 0)
...
ggplot(data_PPC_alt)+
  ...
  #geom_line(aes(zeit,P_cal,col=factor(id,labels = c("1 & 2","3 & 4"))),alpha=0.4)+
  geom_line(aes(zeit,P_roll1,col=factor(id,labels = c("1 & 2","3 & 4"))))+
  #geom_line(aes(zeit,P,col=factor(id,labels = c("1 & 2","3 & 4"))))+
  geom_line(aes(zeit,P_roll2,col=factor(id,labels = c("1 & 2","3 & 4")),linetype="Pmean"))+
  #geom_line(aes(zeit,P_roll_cal,col=factor(id,labels = c("1 & 2","3 & 4")),linetype="Pmean"))+
  facet_wrap(~factor(mode,labels = c(expression(PP),expression("PP &"~P[lateral]))),labeller = label_parsed)+
