...
data_Pico <- rbind(data_Pico_1,data_Pico_2)
names(data_Pico) <- 
...
data_testo_2 <- read.table(paste0(datapfad_testo,"230703_2.csv"),sep = ";",header = T,dec = ".")
data_testo_3 <- read.table(paste0(datapfad_testo,"230704.csv"),sep = ";",header = T,dec = ".")
...
ggplot(data_T)+
  ...
  geom_point(data = subset(data_T,File ==2 &abs(T_diff) < 0.5),aes(date,T_V*38,col="T_V"))+
