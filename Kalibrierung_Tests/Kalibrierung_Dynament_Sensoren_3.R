...
data$date[1]#+1*60*60
#data$date <- data$date+1*60*60

plot(data$date[40:200],data$CO2_Dyn_06[40:200])
points(data$date[40:200],data$CO2[40:200],col=2)
plot(data$date,data$CO2_Dyn_06)
points(data$date,data$CO2,col=2)
...
dyn_list <- wechsel_fun(data=data_dyn,col="CO2_Dyn_02")

GGA_wechsel <- GGA_list[[2]]
dyn_wechsel <- dyn_list[[2]]

GGA_wechsel$start[-1] - dyn_wechsel$start[-1]
t_diff <- mean(as.numeric(difftime(GGA_wechsel$ende[-nrow(dyn_wechsel)],dyn_wechsel$ende[-nrow(dyn_wechsel)],unit="secs")))

data_dyn$date <- data_dyn$date + t_diff

data<-merge(GGA_list[[1]],dyn_list[[1]],all=T)

datelim1 <- ymd_hm(c("2020.04.24 13:00","2020.04.24 16:00"))
...
ggplot(data)+geom_point(aes(date,CO2_Dyn_02,col=kal_punkte))+geom_line(aes(date,CO2dry,col="GGA"))+xlim(datelim2)
