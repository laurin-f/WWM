...
klima_wind <- dataDWD(link[[1]],dir=datapfad_FVAgarten,varnames=T)
klima_P <- dataDWD(link[[2]],dir=datapfad_FVAgarten,varnames=T)

klima <- merge(klima_P[,c(2,4,5,6)],klima_wind[,c(2,4,5)])
klima$date <- ymd_hms(klima$MESS_DATUM)
klima <- klima[,-1]

range <- ymd_hms(c("2021-03-30 16:11:00 UTC", "2021-05-17 09:56:00 UTC"))
klima <- subset(klima,date > range[1] & date < range[2])
...
save(klima,file = paste(datapfad_FVAgarten,"klima_DWD.RData"))
